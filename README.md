...
При `YANDEX_RASP_ENABLED=true` backend регистрирует provider `yandex_rasp` и обращается только к официальному API Яндекс Расписаний (`https://api.rasp.yandex-net.ru/v3.0/search/` и `https://api.rasp.yandex-net.ru/v3.0/stations_list/`). Устаревшее значение `api.rasp.yandex.net` в `YANDEX_RASP_BASE_URL` автоматически заменяется каноническим адресом. Ключ берётся из `YANDEX_RASP_API_KEY`, таймаут отдельного HTTP-запроса — из `YANDEX_RASP_TIMEOUT_SECONDS`. Поиск расписания сначала использует выбранный city/station code и ограничен настройками `YANDEX_MAX_DIRECT_REQUESTS_PER_SEARCH` (12), `YANDEX_MAX_TRANSFER_REQUESTS_PER_SEARCH` (12), `YANDEX_MAX_STATIONS_PER_CITY` (6) и общим deadline `YANDEX_ROUTE_SEARCH_TOTAL_TIMEOUT_SECONDS` (20 секунд). Полный каталог станций при этом остаётся доступен autocomplete. Если provider выключен, используются существующие Mock/GTFS/RZD provider без изменения их поведения.

Загрузка `stations_list` не блокирует запуск API: сначала используется существующий SQLite/JSON cache, а при его отсутствии — встроенный каталог основных городов; полный каталог обновляется в фоне с соблюдением cooldown после ошибок. Для smoke-тестов можно установить `YANDEX_STATIONS_STARTUP_NETWORK_ENABLED=false`, чтобы полностью запретить только фоновый startup-запрос `stations_list`; запросы расписания через Yandex provider при этом не отключаются. Состояние каталога (`ready`/`degraded`, источник, количество записей, последняя ошибка и cooldown) доступно в `/api/v1/providers/health` без API-ключа.
...
Backend сохраняет полный response body только в artifact-файл на диске в `/tmp/business-trip-planner-diagnostics/` и никогда не возвращает полный `raw_body` через публичный API. По умолчанию `YANDEX_DIAGNOSTICS_VERBOSE=false`, поэтому `provider_errors.yandex_rasp.details` содержит компактный набор безопасных полей: HTTP-статус, content-type, content-encoding, ключи JSON-ответа, размер body и пути к artifact-файлам.

Настройки безопасной диагностики:

```env
YANDEX_DIAGNOSTICS_VERBOSE=false
YANDEX_DIAGNOSTICS_PREVIEW_CHARS=4000
YANDEX_DIAGNOSTICS_MAX_DETAILS_BYTES=32768
```
