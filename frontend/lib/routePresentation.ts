...
export function routeSearchNotice(data: RouteSearchResponse, strictAvailability: boolean): { kind: "api" | "empty" | "error"; text: string } {
  ...
  const providerErrors = data.provider_errors ?? data.search_summary?.provider_errors;
  if (providerErrors && Object.keys(providerErrors).length > 0) {
    return { kind: "error", text: "Источник расписаний временно недоступен. Попробуйте повторить поиск позже." };
  }
